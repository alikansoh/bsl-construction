...
export default function BlogsPage() {
  ...
  // Delete flow: which blog is pending confirmation, and delete state
  ...
  async function confirmDelete() {
    ...
    try {
      const response = await fetch(
        `/api/blogs/${encodeURIComponent(pendingDelete.slug)}`,
        {
          method: "DELETE",
          credentials: "include",
        }
      ...
      setBlogs((prev) =>
        prev.filter((blog) => blog.slug !== pendingDelete.slug)
      );
    ...
    <div className="min-h-full bg-[#f6f7f9]">
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard label="Total Posts" value={blogs.length} />
          ...
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-xs">
              <input
                ...
                placeholder="Search by title, category, author, tag or slug..."
        ...
        {!loading && !error && blogs.length === 0 && (
          <div className="rounded-2xl border border-dashed border-black/10 bg-white px-6 py-20 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm text-gray-500">
              Create your first blog post to start sharing your story.
            </p>
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {visibleBlogs.map((blog) => (
              <BlogCard
                key={blog._id}
                blog={blog}
                onDeleteRequest={() => requestDelete(blog)}
              />
            ))}
          </div>
        )}
        ...
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-400">
              {metaLine || `Order: ${blog.displayOrder}`}
            </p>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        >
          Delete &ldquo;{blog.title}&rdquo;?
        </h2>

        <p className="mt-2 text-sm leading-6 text-gray-500">
          This will permanently remove this blog post from your website.
