...
interface ProjectForm {
  ...
  /*
    Same gallery-pool pattern as the create page: every photo lives in
    `gallery`, and thumbnailUrl/heroImageUrl just point at one of them.
    Because the API stores thumbnail/heroImage separately (and strips them
    out of the saved `gallery` array to avoid duplicates), we re-merge them
    back into one pool on load — see `projectToForm` below.
  */
  gallery: ImageField[];
  thumbnailUrl: string;
  heroImageUrl: string;
...
/* Raw shape returned by GET /api/projects/[slug] */
...
/*
  ...
  and hero image are stored separately, and the plain `gallery` array never
  contains them (the create page filters them out before saving), so we
  stitch all three back together here, deduping by url just in case.
*/
function projectToForm(project: ProjectApiResponse): ProjectForm {
  ...
  return {
    ...
    heroImageUrl: project.heroImage?.url ?? "",
    ...
    challenges: project.challenges ?? { title: "Challenges", items: [] },
    solutions: project.solutions ?? { title: "Solutions", items: [] },
    results: project.results ?? { title: "Results", items: [] },
...
/*
  Every field checked here is required by PUT /api/projects/[slug] before
  a project can go live. This list also powers the "Required to publish"
...
/*
  ...
  section each problem belongs to. Mirrors what PUT /api/projects/[slug]
  actually requires. (Slug isn't checked here — it's immutable once the
  project exists, the API strips any slug sent in the body.)
*/
...
/* ----------------------------------------------------------------------- */
/* Small components (identical to the create page)                         */
/* ----------------------------------------------------------------------- */
    ...
    <>
      ...
      <div className="space-y-2">
        {value.items.map((item, index) => (
          <div key={`${item}-${index}`} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2">
            <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#D98E1F]" />
            <span className="flex-1 text-sm text-slate-700">{item}</span>
            <IconButton label="Remove" tone="danger" onClick={() => removeItem(index)} />
          </div>
        ))}
...
export default function EditProjectPage() {
  ...
  */
  ...
  /* Load the existing project once we know the slug. */
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`/api/projects/${originalSlug}`);
  ...
  const updateNested = <K extends keyof ProjectForm>(
    ...
    setForm((current) =>
      current
        ? {
            ...current,
            [key]: { ...(current[key] as object), ...patch } as ProjectForm[K],
          }
        : current,
    );
  };
  ...
  const handleGalleryChange = (gallery: ImageField[]) => {
    setForm((current) => {
      ...
      const stillHasHero = gallery.some((image) => image.url === current.heroImageUrl);
  ...
  const addDetail = () => {
    if (!form) return;
    update("projectDetails", [...form.projectDetails, { label: "", value: "" }]);
  };
  ...
  const sectionComplete = useMemo<Record<NavId, boolean>>(() => {
    ...
    return {
      basics: !!form.title && !!form.category,
      images: !!form.thumbnailUrl && !!form.heroImageUrl,
      overview: !!form.shortDescription && !!form.overview.title && !!form.overview.content,
  ...
  const handleSubmit = async (status: "draft" | "published") => {
    ...
    const heroImage = findImage(form.gallery, form.heroImageUrl) ?? { url: "", alt: "" };

    // Same rule as the create page: the thumbnail/hero photo can still
    // appear in the public gallery, we only drop it from the *saved*
    // gallery array so the live page doesn't show the same photo twice.
    ...
    try {
      const response = await fetch(`/api/projects/${originalSlug}`, {
        ...
        body: JSON.stringify({
          ...
          cta: form.cta.title || form.cta.content ? form.cta : undefined,
      ...
      setFeedback({
        type: "error",
        text: error instanceof Error ? error.message : "Couldn't save the project.",
      });
    } finally {
      setSaving(null);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "error",
        text: error instanceof Error ? error.message : "Couldn't delete the project.",
      });
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="max-w-sm rounded-xl border border-[#C1401F]/30 bg-white p-6 text-center shadow-sm">
          <h1 className="text-base font-semibold text-[#1C2024]">Couldn&apos;t load this project</h1>
  ...
  const previewData = {
    ...
    heroImage: findImage(form.gallery, form.heroImageUrl) ?? { url: "", alt: "" },
    gallery: form.gallery.filter(
      (image) => image.url !== form.thumbnailUrl && image.url !== form.heroImageUrl,
    ),
    overview: form.overview,
    challenges: form.challenges,
    solutions: form.solutions,
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-[1100px] px-4 py-8 sm:px-6">
        <div className="space-y-6">
          ...
          >
            ...
            <Field label="Overview title" required>
              <TextInput
                ...
                onChange={(event) => updateNested("overview", { title: event.target.value })}
              />
            </Field>
            ...
            <Field label="Overview content" required>
              <RichTextEditor
                ...
                onChange={(html) => updateNested("overview", { content: html })}
              />
            </Field>
          </SectionCard>

          <SectionCard id="challenges" n="04" title="Challenges" description="What made this project difficult.">
            <ListBlockEditor
          ...
          <SectionCard id="solutions" n="05" title="Solutions" description="How the challenges were addressed.">
            <ListBlockEditor
          ...
          <SectionCard id="results" n="06" title="Results" description="The outcomes delivered.">
            <ListBlockEditor
              value={form.results}
            ...
            <div className="space-y-3">
              {form.projectDetails.map((detail, index) => (
                <div key={index} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                  ...
                  <IconButton label="Remove" tone="danger" onClick={() => removeDetail(index)} />
                </div>
              ))}
            </div>
          ...
          >
            <Field label="Title">
              <TextInput
                ...
                onChange={(event) => updateNested("cta", { title: event.target.value })}
              />
            </Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Button label">
                <TextInput
                  ...
                  onChange={(event) => updateNested("cta", { buttonLabel: event.target.value })}
                />
              </Field>
              ...
              <Field label="Button link">
                <TextInput
                  ...
                  onChange={(event) => updateNested("cta", { buttonHref: event.target.value })}
                />
              </Field>
            </div>
          </SectionCard>
          ...
          <div className="flex items-center justify-between gap-3 pb-8">
            <div>
              {!confirmingDelete ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-500">Delete this project? This can&apos;t be undone.</span>
        ...
        >
          <div className="flex h-[calc(100dvh-1.5rem)] w-full max-w-[1440px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:h-[calc(100dvh-3rem)]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto bg-[#FAF7F2]">
              <ProjectPreview focusSection={focusSection} data={previewData} />
            </div>
          </div>
        </div>
      )}
